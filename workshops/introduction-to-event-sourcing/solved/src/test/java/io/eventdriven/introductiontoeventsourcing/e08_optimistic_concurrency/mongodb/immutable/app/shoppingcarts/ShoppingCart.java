...
import static io.eventdriven.introductiontoeventsourcing.e08_optimistic_concurrency.mongodb.immutable.app.shoppingcarts.ShoppingCartEvent.*;
...
public record ShoppingCart(
  ...
  public static ShoppingCart evolve(ShoppingCart state, ShoppingCartEvent event) {
    return switch (event) {
      ...
      case ProductItemAddedToShoppingCart(var id, var productItem) ->
        new ShoppingCart(
          state.id,
          state.clientId,
          state.status,
      ...
      case ProductItemRemovedFromShoppingCart(var removed, var productItem) ->
        new ShoppingCart(
          state.id,
          state.clientId,
          state.status,
      ...
      case ShoppingCartCanceled cancelled ->
        new ShoppingCart(
          state.id,
          state.clientId,
          Status.Canceled,
