...
public class EventStore {
  ...
  public <Event> void append(UUID streamId, Event event) {
    events.compute(streamId, (stream, events) -> {
      ...
      var eventHandlers = handlers.get(eventEnvelope.getClass().getTypeName());
  ...
  public <Event> void subscribe(Class<EventEnvelope<Event>> eventClass, Consumer<EventEnvelope<Event>> handler) {
    handlers.compute(eventClass.getTypeName(), (eventType, consumers) -> {
      if (consumers == null)
        consumers = new ArrayList<>();

      consumers.add((event) -> handler.accept(eventClass.cast(event)));
