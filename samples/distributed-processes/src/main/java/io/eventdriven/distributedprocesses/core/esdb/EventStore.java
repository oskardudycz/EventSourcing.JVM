...
public class EventStore {
    ...
    try {
      ...
      if (e.getCause() instanceof WrongExpectedVersionException wrongExpectedVersionException) {
        return new AppendResult.StreamAlreadyExists(
            wrongExpectedVersionException.getActualVersion());
      }
  ...
  public AppendResult append(String streamId, ExpectedRevision expectedRevision, Object... events) {
    try {
      ...
      if (e.getCause() instanceof WrongExpectedVersionException wrongExpectedVersionException) {
        return new AppendResult.Conflict(
            expectedRevision, wrongExpectedVersionException.getActualVersion());
      }
      return new AppendResult.UnexpectedFailure(e);
    }
  }
  ...
  public DeleteResult deleteStream(String streamId) {
    try {
      eventStore
          .deleteStream(
              streamId, DeleteStreamOptions.get().expectedRevision(ExpectedRevision.streamExists()))
          .get();
  ...
  public DeleteResult deleteStream(String streamId, ExpectedRevision expectedRevision) {
    try {
      eventStore
          .deleteStream(streamId, DeleteStreamOptions.get().expectedRevision(expectedRevision))
          .get();
  ...
  public sealed interface ReadResult {
    record Success(ResolvedEvent[] events) implements ReadResult {}
  ...
  public sealed interface AppendResult {
    record Success(ExpectedRevision nextExpectedRevision, Position logPosition)
        implements AppendResult {}

    record StreamAlreadyExists(ExpectedRevision actual) implements AppendResult {}

    record Conflict(ExpectedRevision expected, ExpectedRevision actual) implements AppendResult {}
  ...
  public sealed interface DeleteResult {
    ...
    record Conflict(ExpectedRevision expected, ExpectedRevision actual) implements DeleteResult {}
