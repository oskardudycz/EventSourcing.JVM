  ...
  static GroupCheckout evolve(GroupCheckout current, GroupCheckoutEvent event) {
    return switch (event) {
      case GroupCheckoutInitiated initiated:
        {
          if (!(current instanceof GroupCheckout.Initial)) yield current;
      ...
      case GuestCheckoutsStarted checkoutsStarted:
        {
          if (!(current instanceof GroupCheckout.InProgress inProgress)) yield current;
          ...
          for (var guestStayAccountId : checkoutsStarted.guestStayAccountIds()) {
            if (valueIs(
                guestStayCheckouts, guestStayAccountId, GroupCheckout.CheckoutStatus.Initiated))
              guestStayCheckouts.replace(
                  guestStayAccountId, GroupCheckout.CheckoutStatus.InProgress);
          }

          yield new GroupCheckout.InProgress(inProgress.id(), guestStayCheckouts);
        }
      case GuestCheckoutCompleted checkoutCompleted:
        {
          ...
          if (valueIs(
              guestStayCheckouts,
              guestStayAccountId,
              GroupCheckout.CheckoutStatus.Initiated,
              GroupCheckout.CheckoutStatus.InProgress))
      ...
      case GroupCheckoutCompleted ignore:
        {
          ...
          yield new GroupCheckout.Completed(inProgress.id());
        }
      case GroupCheckoutFailed ignore:
        {
          if (!(current instanceof GroupCheckout.InProgress inProgress)) yield current;

          yield new GroupCheckout.Failed(inProgress.id());
        }
    };
  }
}
