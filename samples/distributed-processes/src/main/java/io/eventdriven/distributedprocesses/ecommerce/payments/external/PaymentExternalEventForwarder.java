...
public class PaymentExternalEventForwarder {
  ...
  public PaymentExternalEventForwarder(
    AggregateStore<Payment, PaymentEvent, UUID> store,
    EventBus eventBus
  ) {
    this.store = store;
  ...
  public void on(PaymentCompleted event) {
    var payment = store.get(event.paymentId())
      .orElseThrow(() -> new IllegalStateException("Cannot enrich event, as payment with id '%s' was not found".formatted(event.paymentId())));
